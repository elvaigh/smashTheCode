...
string tostring(const genome& gen)
{
	string res;
	for (int i(0); i < gen.size(); i++)
		res += to_string(gen.at(i));
	return res;
}
...
#define TAILLE_POP 30 // size of the pop
#define ELLITISM 3 // number of element to save
#define MUT_RATE 0.60 // mut rate
#define CROSS_RATE 0.30 // crossover rate

pair<int, int> pickfirstcorrect() {
	for (int i(0); i < 6; i++)
		for (int j(0); j < 4; j++)
		{
			int pos11(-1), pos22(-1); char aa, bb;
...
int selection(vector<genome>& pop, map<string, int>& allgenome)
{
	...
	{
		string resume = tostring(pop[i]);
		int sc = (allgenome.count(resume) > 0 ? allgenome[resume] : fitness(pop[i])); // get the score, keep the map
		allgenome[resume] = sc;														  // update
		score.push_back(make_pair(sc, i));
		sum += sc;
	}
	// Keep the best element at the beginning, sort the "elitism first best element" by fitness
...
void algoGen(vector<genome>& pop, genome& bestgen, map<string, int> allgenome = map<string, int>())
{
	shiftpop(pop);
	...
	while (!getDiffTime())
	{
		crossover(pop);
		mutatepop(pop);
		int bestscore = selection(pop, allgenome);
		bestgenome[bestscore] = pop[0];
		k++;
	}
	int i(0);
	vector<genome> newpop;
	for (auto x = bestgenome.rbegin(); x != bestgenome.rend(); x++)
	{
		newpop.push_back(x->second);
		if (++i == TAILLE_POP / 2)
			break;
	}
	while (i < TAILLE_POP)
	{
		// newpop.push_back(pop[rand() % pop.size()]); i++;
		genome a(2 * depth);
		randomize(a);
		newpop.push_back(a);
		i++;
	...
	cerr << "Best score : " << bestgenome.rbegin()->first << endl;
	cerr << "Genome : ";
...
void output(const genome& gen)
{
	int pos1(-1), pos2(-1); char a, b;
	transform(mymap, 0, gen[0], gen[1], pos1, pos2, a, b);
    // If the move is impossible? print first correct
	if (pos2 == -1 || pos2 == -1)
	{
		auto res = pickfirstcorrect();
		cout << res.first << " " << res.second << endl;
	}
	else
		cout << gen[0] << " " << gen[1] << endl;
}
